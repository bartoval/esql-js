...
export interface ParseResult<T extends ESQLProperNode = ESQLAstQueryExpression> {
  ...
  /**
   * List of parsed commands. Defined only when the parsed target is a full
   * query; `undefined` for other targets (expression, command, map).
   *
   * @deprecated Use `root` instead.
   */
  ast: T extends ESQLAstQueryExpression ? ESQLAst : ESQLAst | undefined;
...
export class Parser {
  ...
  public static readonly parseHeaderCommand = (
    src: string,
    options?: ParseOptions
  ): ParseResult<ESQLAstHeaderCommand> & { ast: ESQLAst } => {
    ...
    if (!result.errors.length) {
      ...
      if (header && header.length > 0) {
        ...
        return {
          ...
          root: setCommand,
          // @deprecated Use `root` instead.
          ast: [setCommand as unknown as ESQLCommand],
        };
      }
    }
  ...
  public static readonly parseCommand = (
    src: string,
    options?: ParseOptions
  ): ParseResult<ESQLCommand> & { ast: ESQLAst } => {
  ...
  public parseTarget<T extends ESQLProperNode>([
    ...
    const result: ParseResult<T> = {
      ...
      // @deprecated Use `root` instead.
      ast: (isQuery(root) ? root.commands : undefined) as ParseResult<T>['ast'],
    };
